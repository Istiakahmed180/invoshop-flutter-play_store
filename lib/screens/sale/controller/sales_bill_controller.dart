...
class SalesBillController extends GetxController {
  ...
  Future<void> postSaleCreate({
    ...
    final Map<dynamic, dynamic> requestBody = {
      ...
      "saleDateAt": DateFormat("yyyy-MM-dd")
          .format(DateTime.parse(selectedDate.toString())),
      "paymentStatus": paymentStatus.toString(),
      "saleStatus": saleStatus.toString(),
    };
    try {
      final String url = await ApiPath.postSaleCreateEndpoint();
      final response = await _networkService.post(url, requestBody);
      isLoading.value = false;
      if (response.status == Status.completed) {
        Fluttertoast.showToast(
            msg: 'Sale create successfully.',
            backgroundColor: AppColors.groceryPrimary);
        product.clear();
        Get.offNamed(BaseRoute.home);
      } else {
        _showErrorToast(response.message ?? 'Failed to posting sale create');
      }
    } catch (e) {
      isLoading.value = false;
      _showErrorToast('An error occurred while posting sale create');
    }
  }

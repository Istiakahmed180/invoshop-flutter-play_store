...
class _CustomerReportState extends State<CustomerReport> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const CustomAppBar(appBarName: "Customer Reports"),
      body: Obx(
        () => reportsController.isLoading.value
            ? const CustomLoading(
                withOpacity: 0.0,
              )
            : reportsController.customerReportList.isEmpty
                ? Center(
                    child: Text(
  ...
  List<DataColumn> _buildTableColumns() {
    return [
      ...
      _buildTableHeader("Customer Phone"),
  ...
  List<DataRow> _buildTableRows() {
    return reportsController.customerReportList.asMap().entries.map((entry) {
      ...
      return DataRow(
        cells: [
          ...
          _buildDataCell(
              report.customerName != null && report.customerName!.isNotEmpty
                  ? report.customerName ?? "N/A"
                  : "N/A"),
          _buildDataCell(
              report.customerPhone != null && report.customerPhone!.isNotEmpty
                  ? report.customerPhone ?? "N/A"
                  : "N/A"),
          _buildDataCell(

...
class _PurchaseReportState extends State<PurchaseReport> {
  ...
  void initState() {
    super.initState();
    reportsController.getPurchaseReports();
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const CustomAppBar(appBarName: "Purchase Reports"),
      body: Obx(
        () => reportsController.isLoading.value
            ? const CustomLoading(
                withOpacity: 0.0,
              )
            : reportsController.purchaseReportList.isEmpty
                ? Center(
                    child: Text(
  ...
  List<DataRow> _buildTableRows() {
    return reportsController.purchaseReportList.asMap().entries.map((entry) {
      ...
      return DataRow(
        cells: [
          ...
          _buildDataCell(report.unit != null && report.unit!.isNotEmpty
              ...
              : "N/A"),
          _buildDataCell(
            (report.companyName != null && report.companyName!.isNotEmpty

...
class _UserReportState extends State<UserReport> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const CustomAppBar(appBarName: "User Reports"),
      body: Obx(
        () => reportsController.isLoading.value
            ? const CustomLoading(
                withOpacity: 0.0,
              )
            : reportsController.userReportList.isEmpty
                ? Center(
                    child: Text(
  ...
  List<DataRow> _buildTableRows() {
    return reportsController.userReportList.asMap().entries.map((entry) {

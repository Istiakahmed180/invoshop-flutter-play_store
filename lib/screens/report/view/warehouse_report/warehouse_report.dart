...
class _WarehouseReportState extends State<WarehouseReport> {
  ...
  void initState() {
    super.initState();
    reportsController.getWarehouseReports();
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const CustomAppBar(appBarName: "Warehouse Reports"),
      body: Obx(
        () => reportsController.isLoading.value
            ? const CustomLoading(
                withOpacity: 0.0,
              )
            : reportsController.warehouseReportList.isEmpty
                ? Center(
                    child: Text(
  ...
  List<DataRow> _buildTableRows() {
    return reportsController.warehouseReportList.asMap().entries.map((entry) {
      final index = entry.key;
      final report = entry.value;
      return DataRow(
        cells: [
          DataCell(Center(child: Text('${index + 1}'))),
          _buildDataCell(report.name != null && report.name!.isNotEmpty
              ? report.name ?? "N/A"
              : "N/A"),
          _buildDataCell(report.phone != null && report.phone!.isNotEmpty
              ? report.phone ?? "N/A"
              : "N/A"),
          _buildDataCell(
            (report.email != null && report.email!.isNotEmpty
                ? report.email.toString()
                : "N/A"),
          ),
          _buildDataCell(

...
class _SupplierReportState extends State<SupplierReport> {
  ...
  void initState() {
    super.initState();
    reportsController.getSupplierReports();
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const CustomAppBar(appBarName: "Supplier Reports"),
      body: Obx(
        () => reportsController.isLoading.value
            ? const CustomLoading(
                withOpacity: 0.0,
              )
            : reportsController.supplierReportList.isEmpty
                ? Center(
                    child: Text(
  ...
  List<DataRow> _buildTableRows() {
    return reportsController.supplierReportList.asMap().entries.map((entry) {
      ...
      return DataRow(
        cells: [
          ...
          _buildDataCell(
              report.supplierName != null && report.supplierName!.isNotEmpty
                  ? report.supplierName ?? "N/A"
                  : "N/A"),
          _buildDataCell(
              report.supplierCode != null && report.supplierCode!.isNotEmpty
                  ? report.supplierCode ?? "N/A"
                  : "N/A"),
          _buildDataCell(
          ...
          _buildDataCell(
            (report.products != null && report.products!.isNotEmpty
                ? report.products.toString()
                : "N/A"),
          ),
          _buildDataCell(
            (report.purchaseDate != null && report.purchaseDate!.isNotEmpty
                ? report.purchaseDate.toString()
                : "N/A"),
          ),
          _buildDataCell(
            (report.totalAmount != null && report.totalAmount!.isNotEmpty

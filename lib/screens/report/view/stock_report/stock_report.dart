...
class _StockReportState extends State<StockReport> {
  ...
  void initState() {
    super.initState();
    reportsController.getStockReports();
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const CustomAppBar(appBarName: "Stock Reports"),
      body: Obx(
        () => reportsController.isLoading.value
            ? const CustomLoading(
                withOpacity: 0.0,
              )
            : reportsController.stockReportList.isEmpty
                ? Center(
                    child: Text(
  ...
  List<DataRow> _buildTableRows() {
    return reportsController.stockReportList.asMap().entries.map((entry) {
      ...
      return DataRow(
        cells: [
          ...
          _buildDataCell(
              report.stock != null ? report.stock.toString() : "N/A"),

...
class _TaxReportState extends State<TaxReport> {
  ...
  void initState() {
    super.initState();
    reportsController.getTaxReports();
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const CustomAppBar(appBarName: "Tax Reports"),
      body: Obx(
        () => reportsController.isLoading.value
            ? const CustomLoading(
                withOpacity: 0.0,
              )
            : reportsController.taxReportList.isEmpty
                ? Center(
                    child: Text(
  ...
  List<DataColumn> _buildTableColumns() {
    return [
      ...
      _buildTableHeader("Warehouse Name"),
      _buildTableHeader("Invoice No"),
      _buildTableHeader("Tax Type"),
  ...
  List<DataRow> _buildTableRows() {
    return reportsController.taxReportList.asMap().entries.map((entry) {
      ...
      return DataRow(
        cells: [
          ...
          _buildDataCell(
            (report.taxType != null && report.taxType!.isNotEmpty
                ? report.taxType.toString()
                : "N/A"),
          ),
          _buildDataCell(
          ...
          _buildDataCell(
            (report.tax != null && report.tax!.isNotEmpty
                ? report.tax.toString()
                : "N/A"),
          ),
        ],

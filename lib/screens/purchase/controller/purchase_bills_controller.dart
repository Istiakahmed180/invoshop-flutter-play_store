...
class PurchaseBillsController extends GetxController {
  ...
  final RxString purchaseStatus = "".obs;
  ...
  Future<void> postPurchaseCreate({
    ...
    final Map<dynamic, dynamic> requestBody = {
      ...
      "PurchaseDateAt": DateFormat("yyyy-MM-dd")
          .format(DateTime.parse(selectedDate.toString())),
      "purchaseStatus": purchaseStatus.toString(),
    };
    try {
      final String url = await ApiPath.postPurchaseCreateEndpoint();
      final response = await _networkService.post(url, requestBody);
      isLoading.value = false;
      if (response.status == Status.completed) {
        Fluttertoast.showToast(
            msg: 'Purchase create successfully.',
            backgroundColor: AppColors.groceryPrimary);
        product.clear();
        Get.offNamed(BaseRoute.home);
      } else {
        _showErrorToast(
            response.message ?? 'Failed to posting purchase create');
      }
    } catch (e) {
      isLoading.value = false;
      _showErrorToast('An error occurred while posting purchase create');
    }
  }

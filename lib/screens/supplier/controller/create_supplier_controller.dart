...
class CreateSupplierController extends GetxController {
  ...
  Future<void> postSupplierCreate() async {
    isLoading.value = true;
    try {
      final String? userId = prefs.getString("user_id");
      final String url = await ApiPath.postSupplierCreateEndpoint();
      ...
      if (response.status == Status.completed) {
        ...
        _showErrorToast(
            response.message ?? 'Failed to creating supplier account');
      }
    } catch (e) {
      isLoading.value = false;
      _showErrorToast('An error occurred while creating supplier account');
    }
  }

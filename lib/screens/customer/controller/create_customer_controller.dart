...
class CreateCustomerController extends GetxController {
  ...
  Future<void> postCustomerCreate() async {
    isLoading.value = true;
    try {
      final String? userId = prefs.getString("user_id");
      final String url = await ApiPath.postCustomerCreateEndpoint();
      ...
      if (response.status == Status.completed) {
        ...
        _showErrorToast(
            response.message ?? 'Failed to creating customer account');
      }
    } catch (e) {
      isLoading.value = false;
      _showErrorToast('An error occurred while creating customer account');
    }
  }

  void resetCreateCustomerFields() {
    categoryId.value = 0;
    countryId.value = 0;

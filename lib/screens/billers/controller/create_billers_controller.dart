...
class CreateBillersController extends GetxController {
  ...
  Future<void> postBillerCreate() async {
    isLoading.value = true;
    try {
      final String? userId = prefs.getString("user_id");
      final String url = await ApiPath.postBillerCreateEndpoint();
      ...
      if (response.status == Status.completed) {
        Fluttertoast.showToast(
            msg: "Biller create successfully",
        ...
        _showErrorToast(
            response.message ?? 'Failed to creating Biller account');
      }
    } catch (e) {
      isLoading.value = false;
      _showErrorToast('An error occurred while creating Biller account');
    }
  }

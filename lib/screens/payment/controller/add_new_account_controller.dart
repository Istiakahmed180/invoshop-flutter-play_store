...
import 'package:ai_store/screens/payment/model/all_bank_model.dart';
...
class AddNewAccountController extends GetxController {
  ...
  Future<void> postBankAccountCreate() async {
    isLoading.value = true;
    try {
      final String? supplierID = prefs.getString("supplier_id");
      final String url = await ApiPath.postBankAccountCreateEndpoint();
      ...
      if (response.status == Status.completed) {
        Fluttertoast.showToast(
            msg: "Bank account create successfully",
            backgroundColor: AppColors.groceryPrimary);
        resetForm();
        Get.back();
      } else {
        _showErrorToast(response.message ?? 'Failed to creating bank account');
      }
    } catch (e) {
      isLoading.value = false;
      _showErrorToast('An error occurred while creating bank account');
    }
  }
